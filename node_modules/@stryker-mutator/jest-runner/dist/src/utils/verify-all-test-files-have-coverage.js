...
exports.verifyAllTestFilesHaveCoverage = void 0;
 ...
 */
function verifyAllTestFilesHaveCoverage(results, fileNamesWithMutantCoverage) {
    const allTestFiles = new Set(results.testResults.map(({ testFilePath }) => path_1.default.resolve(testFilePath)));
    const fullFileNamesWithCoverage = fileNamesWithMutantCoverage.map((fileName) => path_1.default.resolve(fileName));
...
exports.verifyAllTestFilesHaveCoverage = verifyAllTestFilesHaveCoverage;
...
function formatError(coverageMissingFromFiles) {
    let fileHints = coverageMissingFromFiles
        ...
        .map((fullName) => `  * ${path_1.default.relative(process.cwd(), fullName)}`)
        .join('\n');
