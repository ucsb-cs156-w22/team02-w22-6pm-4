...
exports.InputFileResolver = exports.MUTATION_RANGE_REGEX = void 0;
...
class InputFileResolver {
    ...
    async expandPattern(globbingExpression, logAboutUselessPatterns) {
        if (exports.MUTATION_RANGE_REGEX.exec(globbingExpression)) {
            globbingExpression = globbingExpression.replace(exports.MUTATION_RANGE_REGEX, '$1');
        }
        const fileNames = (await file_utils_1.glob(globbingExpression)).map((relativeFile) => path_1.default.resolve(relativeFile));
    ...
    async resolveFilesUsingGit() {
        try {
            ...
            const fileNames = relativeFileNames.map((relativeFileName) => path_1.default.resolve(relativeFileName));
            return fileNames;
        }
        catch (error) {
    ...
    async readFiles(fileNames) {
        const promisedFiles = rxjs_1.from(fileNames)
    ...
    async readFile(fileName) {
        try {
            const content = await fs_1.default.promises.readFile(fileName);
